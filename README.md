...
7.  nvm use <version> 成功后，root指定的位置会出现nodejs的快捷方式；npm就包含在node文件夹中；
  > nvm 可以安装在c盘之外的硬盘盘符中，但不能安装在u盘之中，不要浪费时间了；
...
```
- 将NPM目录配置到其他目录时，必须将该目录放到系统的环境变量中，否则无法再全局使用
  + NPM_HOME = 要配置的目录（C:\dev\nvm\npm\node_modules）；
  + PATH += %NPM_HOME%;
```
...
### 常用NPM命令
 - 本地模式安装模块,一般安装在本地项目中，在代码require中调用；安装完毕后会产生一个node_modules目录，其目录下就是安装的各个node模块。
 ```
 $ npm install <name>
...
- 全局模式安装模块，包会被安装到Node的安装目录下的node_modules下，配置过npm的prexy属性之后，会安装在prexy指向的目录；
```
$ npm install -g <name>

```
> 在代码中，直接通过require()的方式是没有办法调用全局安装的包的。全局的安装是供命令行使用的，就好像全局安装了vmarket后，就可以在命令行中直接运行vm命令
